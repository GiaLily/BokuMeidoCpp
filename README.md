...
- 日志：同步/异步输出到终端或文件，支持**回调模式**——一行 `setupSyncLogger(level, callback)` 即把库内所有日志消息一键桥接到 spdlog 等任意第三方日志库，无需改业务代码
...
**回调模式的价值**：当项目里已经存在一套日志体系（如 spdlog、glog，或统一的远程日志/上报服务）时，无需弃用现有日志库，也无需逐处改写 `MEIDO_*` 调用——只需在启动时给一个回调，库内所有日志（含库自身内部日志）就会自动流入你的日志体系，实现统一采集、统一格式、统一输出。
...
### v1.0.1
*2026-08-21*
1. 优化 `print` 性能：通过 `thread_local` + 自定义缓冲区，消除与 `cout` 交错缓冲区的风险；
2. 优化 `toStr`、`format`、`ObjectPool` 的实现细节；
3. 新增内部使用的跨 DLL 标志位读写接口，用于调试；
4. 其他非代码杂项优化。
